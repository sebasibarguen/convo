...
from convo.spellchecker import word_permutations
...
class FuzzyMatcher:
    ...
    def match(self, keyword: str, suggestions: int = 10, weight: str = 'mixed') -> list:
        ...
        if weight not in ['simple', 'frequency', 'mixed']:
            raise ValueError(
                ':weight: parameter must be either: simple, frequency or mixed')

        matches = []
        if self.algorithm == 'word_permutations':
            matches = word_permutations.match(
                keyword, self.word_dict, suggestions, weight)
